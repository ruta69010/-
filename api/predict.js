...
export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `競馬AI。渡された出走馬データから実際の馬名・騎手・調教師・斤量・前走成績・発走時刻などを抽出し、それに基づいて分析せよ。オッズはデータ抽出のみ行い、指数評価には一切使用しないこと（オッズは人気であって実力ではない）。データに無い情報の創作・改変は禁止。発走時刻はページ内の「12:10」「15:35」などの時刻表記を必ず正確に抽出してpostTimeに"HH:MM"形式で記載すること。時刻が複数ある場合は対象レースの時刻を選ぶこと。不明な場合のみ空文字にする。
...
${cacheKey.date} ${label} 第${cacheKey.raceNum}R
...
上記データに基づいてJSONを作成せよ。HTMLに記載されている出走馬を全頭漏れなく含めること。前走成績が実際に記載されている馬は新馬として扱わず、必ず指数を算出すること。データに無い項目は妥当な値を補ってよいが、馬名・騎手・調教師・オッズなど実データに含まれる項目は改変しないこと。出走馬の頭数は実データと完全に一致させること。JSONのみ返せ。`;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        if (aiData?.error) {
          lastError = { error: "Gemini APIエラー: " + (aiData.error.message || JSON.stringify(aiData.error)) };
          continue;
        }

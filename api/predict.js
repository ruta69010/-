...
export default async function handler(req, res) {
  ...
  try {
    ...
    const system = `競馬AI。渡された出走馬データから実際の馬名・騎手・調教師・斤量・オッズ・前走成績・発走時刻などを抽出し、それに基づいて分析せよ。データに無い情報の創作・改変は禁止。発走時刻はページ内の「12:10」「15:35」などの時刻表記を必ず正確に抽出してpostTimeに"HH:MM"形式で記載すること。時刻が複数ある場合は対象レースの時刻を選ぶこと。不明な場合のみ空文字にする。新馬戦の定義:出走馬の前走実績が全く無い（1レースも走っていない）場合のみ新馬戦として扱う。2歳馬でも前走実績がある場合は通常通り分析して指数を出すこと。前走実績が無い馬のみrecentIdx・distIdx・trackIdxをnullにし、prevResultsを"新馬"とすること。それ以外の馬は必ず実データから指数を算出すること。各指数は0〜130の範囲で、レースクラスと実力を正直に反映して評価せよ。無理に高い数字を出さず、実データから判断した妥当な数値にすること。以下を目安とするが、あくまで実力に基づいて判断せよ。地方下級条件(C3/C2/C1):上限目安70〜80。地方上級・重賞:上限目安85〜100。JRA条件戦:上限目安85〜105。JRA重賞・G1:上限目安100〜130。レース内で実力差がある場合は数値に差をつけ、横並びにしないこと。recentIdx(近走指数):直近3〜5走の着順・着差・タイム・相手関係を総合評価。distIdx(距離適性):今回距離での過去成績・タイム・末脚持続力から適性評価。trackIdx(馬場適性):今回馬場状態(良/稍重/重/不良)・コース形態での過去成績から評価。オッズだけに依存せず、実データから独立して評価すること。aiScoreも0〜130で算出すること。JSONのみ、前後の説明文は一切不要。各文字列フィールドは指定字数以内で簡潔に。{"raceName":"名","postTime":"11:00","distance":"1400m","surface":"良","analysisNote":"20字以内","horses":[{"num":1,"name":"馬名","jockey":"騎手","trainer":"調教師","weight":55,"bodyWeight":"498(-2)","recentIdx":75,"distIdx":70,"trackIdx":65,"aiScore":73,"odds":3.5,"comment":"20字以内","prevResults":"前走2着","strengths":"8字以内","weaknesses":"8字以内"}]}`;

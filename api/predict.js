...
- 1着・2着が多い馬は80以上、3〜5着中心の馬は50〜70、大敗が続く馬は0〜40を目安に
- 着差が小さい敗戦（僅差負け）は着順より高めに評価し、着差が大きい敗戦は低めに評価する
- 格上挑戦での好走は高評価、格下相手での凡走は低評価とする
- 直近のレースほど重みを大きくする（前走>2走前>3走前）
- recentIdxMaxには、直近5走の中で最も評価の高かった1走の評価値を入れること。前走実績が無い馬はnullにする。

各指数は0〜130の範囲で評価せよ。地方下級条件(C3/C2/C1):上限目安70〜80。地方上級・重賞:上限目安85〜100。JRA条件戦:上限目安85〜105。JRA重賞・G1:上限目安100〜130。

distIdx(距離適性):今回距離での過去成績・タイム・末脚持続力から適性評価。
trackIdx(馬場適性):今回馬場状態(良/稍重/重/不良)・コース形態での過去成績から評価。
aiScoreはrecentIdx・distIdx・trackIdxの実力評価から算出し、オッズは一切参照しないこと。

trackTypeには"芝"または"ダート"を正確に抽出すること。horseCountには実際の出走頭数を入れること。

JSONのみ、前後の説明文は一切不要。{"raceName":"名","postTime":"11:00","distance":"1400m","trackType":"ダート","surface":"良","horseCount":12,"analysisNote":"20字以内","horses":[{"num":1,"name":"馬名","jockey":"騎手","trainer":"調教師","weight":55,"bodyWeight":"498(-2)","recentIdx":75,"recentIdxMax":88,"distIdx":70,"trackIdx":65,"aiScore":73,"odds":3.5,"comment":"20字以内","prevResults":"前走2着","strengths":"8字以内","weaknesses":"8字以内"}]}`;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        if (aiData?.errors?.length > 0) {
          lastError = { error: "Cloudflare AIエラー: " + aiData.errors.map(e => e.message).join(", ") };
          continue;
        }
